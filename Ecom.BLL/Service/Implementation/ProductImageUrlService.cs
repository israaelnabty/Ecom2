            ...
            public async Task<ResponseResult<bool>> CreateAsync(CreateProductImageUrlVM model)
            {
                try
                {
                    ...
                    if (model.Image != null)
                    {
                        try
                        {
                            uploadedImageUrl = await Upload.UploadFileAsync("Images/ProductImages", model.Image);
                        }
                        catch (Exception ex)
                        {
            ...
            public async Task<ResponseResult<bool>> UpdateAsync(UpdateProductImageUrlVM model)
            {
                try
                {
                    ...
                    if (model.Image != null)
                    {
                        try
                        {
                            newImageUrl = await Upload.UploadFileAsync("Images/ProductImages", model.Image);
                            if (!string.IsNullOrEmpty(existing.ImageUrl))
                            {
                                await Upload.RemoveFileAsync("Images/ProductImages", existing.ImageUrl);
                            }
                        }
                        catch (Exception ex)
                        {
